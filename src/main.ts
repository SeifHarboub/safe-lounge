...
function renderMenu(animate = true) {
  ...
  grid.innerHTML = dishes.length ? dishes.map((dish, index) => `<button class="dish-card" data-open="${dish.id}" aria-label="Voir ${escape(dish.name)}, ${dish.price} euros"><div class="dish-photo">${dish.image ? `<img src="${asset(dish.image)}" alt="${escape(dish.name)}" width="1080" height="1080" loading="${index < 3 ? 'eager' : 'lazy'}" />` : `<div class="no-photo"><img src="${asset('/assets/symbol-lilac.svg')}" alt="" /><span>LES RAFRAÎCHISSEMENTS</span></div>`}<span class="dish-open">↗</span></div><div class="dish-title"><h4>${dish.id === 'boissons-5' ? 'Les boissons fraîches' : escape(dish.name)}</h4><span>${dish.price}<small>€</small></span></div><p>${escape(dish.id === 'boissons-5' ? dish.name : dish.description || (category.id === 'milkshakes' ? 'Milkshake' : ''))}</p>${dish.extra ? `<span class="dish-extra">${escape(dish.extra)}</span>` : ''}</button>`).join('') : '<div class="no-results"><span>Rien dans l’assiette… pour le moment.</span><p>Essayez « pizza », « chocolat » ou « poulet ».</p><button type="button" data-clear>Revenir à la carte ↗</button></div>';
...
function openDish(id: string) {
  ...
  $('#dialog-content').innerHTML = `${dish.image ? `<img class="dialog-photo" src="${asset(dish.image)}" alt="${escape(dish.name)}" width="1080" height="1080" />` : `<div class="dialog-photo no-photo"><img src="${asset('/assets/symbol-lilac.svg')}" alt="" /></div>`}<div class="dialog-copy"><p class="eyebrow">${escape(category.label)}</p><h2 id="dialog-title">${dish.id === 'boissons-5' ? 'Les boissons fraîches' : escape(dish.name)}</h2><strong class="dialog-price">${dish.price} €</strong><p>${escape(dish.id === 'boissons-5' ? dish.name : dish.description)}</p>${dish.extra ? `<p class="dialog-extra">${escape(dish.extra)}</p>` : ''}${category.id === 'burgers' ? '<p class="dialog-extra">Servi avec frites allumettes croustillantes.</p>' : ''}<div class="dialog-footer">${category.id === 'hookah' ? 'Espace chicha réservé aux adultes.' : 'Une question sur les allergènes ? Notre équipe vous renseigne.'}</div></div>`;
